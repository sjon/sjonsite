...
$(document).ready(function() {
	...
	$('.wymeditor').wymeditor({
		...
		postInit: function (wym) {
			...
			if (jQuery(wym._element).hasClass('small')) {
				// this is the summary box
				jQuery(wym._box).find(wym._options.iframeSelector).css('height', '100px');
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_h4').remove();
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_h5').remove();
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_h6').remove();
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_pre').remove();
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_blockquote').remove();
				jQuery(wym._box).find(wym._options.containersSelector).find('li.wym_containers_th').remove();
			}
			else {
